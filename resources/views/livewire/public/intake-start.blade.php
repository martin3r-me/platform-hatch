<div class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 flex items-center justify-center p-4">
    <div class="w-full max-w-lg">
        @if($state === 'loading')
            <div class="bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl p-8 text-center">
        ...
        @elseif($state === 'notFound')
            <div class="bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl p-8 text-center">
                ...
                <h1 class="text-xl font-bold text-gray-900 mb-2">Link nicht gefunden</h1>
                <p class="text-gray-600">Dieser Link ist ungültig oder die Umfrage existiert nicht mehr.</p>
            </div>
        @elseif($state === 'notActive')
            <div class="bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl p-8 text-center">
                ...
                <h1 class="text-xl font-bold text-gray-900 mb-2">Umfrage nicht verfügbar</h1>
                <p class="text-gray-600">Die Umfrage <strong>{{ $intakeName }}</strong> ist derzeit nicht aktiv.</p>
            </div>
        @elseif($state === 'notStarted')
            <div class="bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl p-8 text-center">
                ...
                <h1 class="text-xl font-bold text-gray-900 mb-2">Umfrage noch nicht gestartet</h1>
                <p class="text-gray-600">Die Umfrage <strong>{{ $intakeName }}</strong> wurde noch nicht gestartet. Bitte versuchen Sie es später erneut.</p>
            </div>
        @elseif($state === 'ready')
            {{-- Begrüßung --}}
            <div class="text-center mb-8">
                @if($logo = \Platform\Hatch\Support\PublicBranding::logoUrl())
                    <img src="{{ $logo }}" alt="{{ \Platform\Hatch\Support\PublicBranding::appName() }}" class="mx-auto mb-6 h-14 w-14 rounded-2xl object-contain">
                @endif
                <p class="text-sm font-medium text-indigo-600 uppercase tracking-wider">Herzlich willkommen zur Umfrage</p>
                <h1 class="mt-2 text-2xl font-bold text-gray-900">„{{ $intakeName }}“</h1>
                @if($intakeDescription)
                    <p class="mt-3 text-gray-600">{{ $intakeDescription }}</p>
                @endif
            </div>

            <div class="space-y-4">
                {{-- Card 1: Umfrage starten --}}
                <div class="bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl p-6">
                ...
                <div class="bg-white/80 backdrop-blur-sm shadow-2xl rounded-2xl p-6">
                    <h2 class="text-lg font-semibold text-gray-900 mb-2">Schon begonnen?</h2>
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row">
                        <input
                            ...
                            class="w-full min-w-0 flex-1 text-base px-3 py-2.5 bg-white border border-gray-300 rounded-lg font-mono uppercase tracking-widest text-center text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                        />
                        <button
                    ...
                    @if($resumeError)
                        <p class="mt-2 text-sm text-red-600">{{ $resumeError }}</p>
                    @endif
                </div>
            </div>
        @endif
