        ...
        <div class="flex gap-2 {{ $compact ? '' : 'justify-center py-2' }}">
            @for($i = 1; $i <= $maxStars; $i++)
                <button type="button"
                    ...
                    <svg class="{{ $compact ? 'w-7 h-7' : 'w-10 h-10' }} transition-colors {{ $i <= $current ? 'text-amber-400 fill-amber-400' : 'text-gray-200' }}" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="0.5">
